...
public interface WeatherRepository extends JpaRepository<Weather, Long> {
    ...
    Optional<Weather> findLatestByLocation(String location);

    @Query(value = "SELECT * FROM weather WHERE location = :location ORDER BY recorded_at DESC LIMIT 1", nativeQuery = true)
    Optional<Weather> findMostRecentByLocation(@Param("location") String location);
    ...
    List<Weather> findWeatherHistory(@Param("location") String location, @Param("startTime") LocalDateTime startTime,
    ...
    List<Weather> findHighRainfallEvents(@Param("minRainfall") Double minRainfall,
            @Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime);

    List<Weather> findByLocationOrderByRecordedAtDesc(String location);
}